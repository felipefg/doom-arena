    ...
    def get_handler(self, request: RollupResponse):
        ...
        for route in self.routes:
            if request.request_type != route.requestType:
                print(f'{request.request_type=} is not {route.requestType=}')
                continue

            match, params = _match_url(route.path_regex, req_path)
            if not match:
                LOGGER.info("No match between %s and %s", req_path, repr(route.path_regex))
                continue
            LOGGER.info("Matched with %s", repr(route))
